...
# #
# #     Filename         : multi_subject_binary.py
...
def multi_subject_binary_model(filters1, filters2, filters3, units_dense, lr):
...
def multi_subject_binary_fit(model, x_train, x_eval, y_train, y_eval, num):
    callbacks_list = [keras.callbacks.ModelCheckpoint(
        filepath='./model/multi_subject_binary_' + str(num) + '_{epoch:04d}-{val_acc:.5f}.h5',
...
if __name__ == '__main__':
    ...
    x_train, x_eval, x_test, y_train, y_eval, y_test = multi_subject_binary_data()

    for i in range(1, 11):
        model = multi_subject_binary_model(16, 32, 16, 256, 0.0125)
        multi_subject_binary_fit(model, x_train, x_eval, y_train, y_eval, i)
    ...
    x_train, x_eval, x_test, y_train, y_eval, y_test = multi_subject_binary_data()
    path = './model/best_model/multi_subject_binary.h5'
